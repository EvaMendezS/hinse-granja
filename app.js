...
function wireButtons() {
  ...
  // Save buttons — con oferta de PDF después de guardar
  const saves = {
    btnSaveLote:        { fn: saveLote,         label: '🐣 Lote' },
    btnSaveRuptura:     { fn: saveRuptura,      label: '🥚 Ruptura de postura' },
    btnSavePostura:     { fn: savePostura,      label: '🥚 Postura' },
    btnSaveAlimentacion:{ fn: saveAlimentacion, label: '🌾 Alimento' },
    btnSaveVacunacion:  { fn: saveVacunacion,   label: '💉 Vacunación' },
    btnSaveMedicacion:  { fn: saveMedicacion,   label: '💊 Medicación' },
    btnSaveMortandad:   { fn: saveMortandad,    label: '💀 Mortandad' },
    btnSaveEnfermedad:  { fn: saveEnfermedad,   label: '🦠 Enfermedad' },
    btnSaveNota:        { fn: saveNota,         label: '📷 Nota' },
    btnSaveFormula:     { fn: saveFormula,      label: '🌾 Fórmula' },
  };
  Object.entries(saves).forEach(([id, { fn, label }]) => {
    const el=$(id); if(!el) return;
    el.addEventListener('click', () => {
      fn();
      setTimeout(() => ofrecerPDF(label), 400);
    });
  });
...
// ─── PDF AL GUARDAR ───────────────────────────────────────────
function ofrecerPDF(label) {
  const old = $('toastPDF');
  if(old) old.remove();
  const el = document.createElement('div');
  el.id = 'toastPDF';
  el.className = 'toast-pdf';
  el.textContent = '📄 ¿Ver PDF actualizado?';
  document.body.appendChild(el);
  el.addEventListener('click', () => { el.remove(); generarPDF(); });
...
function renderFormulas() {
  ...
  if(!list.length){
    el.innerHTML=`<div class="formulas-empty"><div class="empty-icon">🌾</div><p>Aún no hay fórmulas guardadas.</p><p style="font-size:.82rem;margin-top:6px;color:var(--text3)">Guardá acá las recetas de balanceado por etapa productiva.</p></div>`;
    return;
  }
...
function renderHistorial(loteId) {
  ...
  res.innerHTML=`<div class="data-card" style="margin-bottom:14px">
    ...
    <div class="data-card-actions">
      <button class="btn-pdf-lote" data-pdf-lote="${loteId}" style="background:var(--accent3);color:var(--beige);border:none;border-radius:8px;padding:6px 14px;font-size:.82rem;cursor:pointer;">📄 PDF de este Lote</button>
    </div>
  </div>`;
...
function exportarExcel() {
  ...
  ${makeTable('🌾 FÓRMULAS',['Nombre','Etapa','g/ave/día','Proteína%','kcal/kg','Calcio%','Proveedor'],
    DB.get(KEYS.formulas).map(f=>[f.nombre,ETAPA_NOMBRES[f.etapa]||f.etapa,f.grAve||'',f.proteina||'',f.energia||'',f.calcio||'',f.proveedor||'']))}
  </body></html>`;
